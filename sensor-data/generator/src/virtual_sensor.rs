...
impl<'a> VirtualSensor<'a> {
    ...
    /// Returns a vector containing sensor data generated by the virtual sensor in the timeframe starting at `begin` and ending at `end`.
    ///
    /// # Arguments
    ///
    ...
    pub fn generate_sensor_data_files_json(
        &self,
        begin: u64,
        end: u64,
        ...
        let mut result: Vec<SensorDataFile> = Vec::new();
        ...
        while timestamp < end {
            for signal in self.sensor.signals() {
                ...
                let measurement =
                    measurements_generator.get_measurement(timestamp, 10, signal.quantity);
            ...
            let data = serde_json::to_vec(&json_object).unwrap();
            // create a sensor data file
            let sensor_data_file = SensorDataFile {
                data,
                sensor_id: self.sensor.id.to_string(),
                file_format: Some(self.file_format.clone()),
            };
            // add sensor data file to vector that is to be returned
            result.push(sensor_data_file);
            // make a step in time
...
mod tests {
    ...
    #[test]
    fn test_generate_sensor_data_files_json() {
        ...
        let sensor_data_files = virtual_sensor.generate_sensor_data_files_json(
            timestamp_begin as u64,
            timestamp_end as u64,
            &mut measurements_generator,
        );

        // Check that there are exactly two messages in sensor_data_files
        ...
        for message in &sensor_data_files {
            ...
            assert!(message_data_json.get("Temperature(C)").is_some());
            assert!(message_data_json.get("Weight(g)").is_some());
            assert!(message_data_json.get("electricity(W)").is_some());
        }
    }
}
