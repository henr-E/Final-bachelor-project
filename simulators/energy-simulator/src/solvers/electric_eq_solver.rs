...
use crate::nodes::electric::generic_node::BusType;
...
pub fn distributed_round(
    ...
    if let Some(neighbours) = slack_neighbours {
        for (&key, (voltage, angle)) in &neighbours {
            let line = lines
                .get(&key)
                .ok_or_else(|| format!("Line with key {} not found", key))?;
            sum_yv += line.to_complex()
                * voltage.amplitude
                * (line.conductance * angle.cos() + line.susceptance * angle.sin());
        }
    }
