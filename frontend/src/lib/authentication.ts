...
export async function login(formData: FormData): Promise<Result<string, LoginError>> {
...
export async function register(formData: FormData): Promise<Result<{}, RegisterError>> {
    // Create and validate the user data input schema from form data.
    const user = schema.safeParse({
        username: formData.get('username'),
        password: formData.get('password'),
