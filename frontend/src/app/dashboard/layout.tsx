                ...
                <div className="px-12 py-3 grow h-full flex flex-col">
                    <div className="my-4 flex flex-row">
                        ...
                        <Breadcrumb aria-label="Default breadcrumb example">
                            {
                                pathname.slice(1, pathname.length).split("/").map((item) =>
                                    (item === "dashboard")?
                                        <Breadcrumb.Item key={item} href={"/" + item} icon={HiHome}></Breadcrumb.Item>:
                                        <Breadcrumb.Item key={item} href={pathname.split(item)[0] + item} >{item}</Breadcrumb.Item>)
                            }
                        </Breadcrumb>
                    </div>
                    {children}
