        ...
        <div className="container space-y-4">
            ...
            <div className="space-y-4 flex flex-col w-auto">
                ...
                <div className="shadow-md sm:rounded-lg bg-white p-2 w-full min-h-96 relative">
                    {
                        sensorState.sensors.length === 0 && <div className="absolute flex items-center justify-center w-full h-full"><span className="text-sm text-gray-500">No Sensors</span></div>
                    }
                        ...
                        <tbody>
                            {
                                sensorState.sensors.map(sensor =>
                                    <tr key={sensor.id} className="my-6">
                                        ...
                                        <td className="px-3 max-w-32 truncate">
                                            {sensor.name}
                                        </td>
                                        <td className="px-3 max-w-48 truncate">
                                            {sensor.description}
                                        </td>
                                        ...
                                        <td className="px-3 w-24">
                                            <div className="flex flex-row space-x-3">
                                                ...
                                                <button onClick={() => setSensorToIngest(sensor)}><BsDatabaseFillAdd size={24} /></button>
                                                <button className="text-red-300" onClick={() => setSensorToDelete(sensor)}><MdOutlineDeleteOutline size={24} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                )
            ...
            <CreateSensorModal isModalOpen={isCreateSensorModalOpen} closeModal={() => setIsCreateSensorModalOpen(false)} />
            <DeleteSensorModal sensor={sensorToDelete} confirm={handleConfirmSensorDelete} cancel={() => handleCancelSensorDelete()} />
            <DeleteMultipleSensorsModal isModalOpen={isDeleteMultipleSensorsModalOpen} sensors={selectedSensors} confirm={handleDeleteSelectedSensors} closeModal={handleCancelSelectedSensorsDelete} />
        </div>
    );
}
