...
function SimulationOverviewPage() {
    ...
    return (
        <div className="flex flex-col space-y-4 w-full h-full">
                    ...
                    <div className="shadow-md sm:rounded-lg bg-white p-2 w-full min-h-96 relative">
                        <table className="text-sm text-left rtl:text-right text-gray-500 w-full table-auto">
                            ...
                            <tbody>
                                <tr>
                                    <td>
                                        {!simulations && (<Spinner aria-label="Medium sized spinner example" size="md" />)}
                                    </td>
                                </tr>
                                {
                                    simulations?.item.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-50" onClick={()=>{router.push('simulation/' + item.id);}} style={{cursor: "pointer"}}>
                                            ...
                                            <td className="p-3 px-3">
                                                {new Date(+item.creationDateTime * 1000).toLocaleString()}
                                            </td>
                                            <td className="p-3 px-3">
                                                {item.framesLoaded}
                                            </td>
                                            <td className="p-3 px-3 text-center items-center flex justify-center w-20">
                                                {item.status == 0?
                                                    ...
                                                    item.status == 1?
                                                        <Spinner color="success" aria-label="Medium sized spinner example" size="md" />:
                                                        <Icon path={mdiCheck  } color="green" size={1} className="content-center" />}
                                            </td>
                                            <td className="p-3 px-3 w-16">
                                                <div className="flex flex-row space-x-2 justify-center ">
                                                <button><MdOutlineDeleteOutline size={24} onClick={()=>{alert("remove item")}}/></button>
                                                </div>
                                            </td>
                                        </tr>
                                        ))
            ...
            <CreateSimulationModal isModalOpen={isCreateSensorModalOpen} closeModal={() => {setIsSensorModalOpen(false); loadSimulations();}}></CreateSimulationModal>
        </div>
    );
}
