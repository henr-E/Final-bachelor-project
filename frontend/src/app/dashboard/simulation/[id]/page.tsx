...
import {RangeSlider, Card, Spinner, Label, TextInput, Tooltip, Button, Badge} from 'flowbite-react';
...
function SimulationPage() {
    ...
    const [frames, setFrames] = useState<Array<SimulationFrame>>(Array(maxAvailableFrames).fill(undefined));
        ...
        <div className="flex flex-col h-full">
            <Card className="flex-auto">
                ...
                <div className="flex flex-wrap gap-2">
                    <Badge color="gray">Start time: {(new Date((simulation?.startDateTime || 0) * 1000)).toLocaleString()}</Badge>
                    <Badge color="gray">End time: {(new Date((simulation?.endDateTime || 0) * 1000)).toLocaleString()}</Badge>
                    <Badge color="gray">Status: {simulation?.status}</Badge>
                </div>
                <MapFrame twin={twinState.current} frame={frames[currentTime]} frameNr={currentTime}></MapFrame>
            </Card>
            <Card className="mt-1">
                <div className="flex">
                    <div className="flex">
                    ...
                    <div className="flex">
                        {
                            frames.slice(currentTime, currentTime + settings.bufferSize).filter(function (value) {
                                ...
                                <Tooltip content={`Buffer loading: ${frames.slice(currentTime, currentTime + settings.bufferSize).filter(function (value) {
                                    return value !== undefined
            ...
            {frames[currentTime] && <CreateSimulationModal
                ...
                timeStepDelta={((((simulation?.endDateTime  || 60) - (simulation?.startDateTime || 0))) / (simulation?.framesLoaded || 1))}
                startDate={simulation?.startDateTime? new Date(simulation?.startDateTime * 1000 + 1000 * currentTime * ((((simulation?.endDateTime || 60) - (simulation?.startDateTime))) / (simulation?.framesLoaded || 1))): undefined}
                endDate={simulation?.endDateTime? new Date(simulation?.endDateTime * 1000): undefined}
                startTime={simulation?.startDateTime? new Date(simulation?.startDateTime * 1000 + 1000 * currentTime * ((((simulation?.endDateTime || 60) - (simulation?.startDateTime))) / (simulation?.framesLoaded || 1))).toLocaleTimeString("nl-BE"): undefined}
                endTime={simulation?.endDateTime?  new Date(simulation?.endDateTime * 1000).toLocaleTimeString("nl-BE"): undefined}
