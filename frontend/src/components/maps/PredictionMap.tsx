...
export function PredictionMap({
    ...
    eventHandlers = {},
    mapItems = [],
    mode = PredictionMapMode.RealtimeMode,
}: PredictionMapProps) {
        ...
        <>
            ...
            >
                ...
                {buildings.map((building, index) => (
                    <Polygon
                        key={index}
                        positions={building.geometry.coordinates[0].map(coordinate => [
                            coordinate[1],
                            coordinate[0],
                        ])}
                        color='red'
                    ...
                    >
                        <Popup>
                            <div>
                                Street: {building.properties?.['addr:street'] ?? 'not available'}
                            </div>
                            <div>
                                Housenumber:{' '}
                                {building.properties?.['addr:housenumber'] ?? 'not available'}
                            </div>
                        </Popup>
                    </Polygon>
                ))}
