...
function MapEditor({nodeItemRef, edgeItemRef, noBuildings, initialEdges, initialNodes}: MapEditorProps) {
    ...
    useEffect(() => {
        ...
        if (nodeItemRef)
            nodeItemRef.current = nodes;

        if (edgeItemRef)
            edgeItemRef.current = edges;
    ...
    function selectNode(index: number) {
        if (cursorRef.current === CursorState.CONNECT_ITEMS) {
            ...
            if(item)
                setSelectedItems([...selectedItemsRef.current, item]);
            return;
        }
        let item = nodesRef.current.get(index);
        if(item)
    ...
    const addLine = (id: number) => {
        ...
        (edgesRef.current[edgesRef.current.length - 1] as LineItem).items.push(nodesRef.current.get(id) as NodeItem);
    }
    ...
    const onSelectBuilding = (building: BuildingItem) => {
        ...
        if (cursorRef.current === CursorState.NONE || cursorRef.current === CursorState.GRAB || cursorRef.current === CursorState.CONNECT_ITEMS || nodesRef.current.get(building.id)) {
            if(nodesRef.current.get(building.id))
        ...
        try {
            if(selectedItemsRef.current[0].type == 2){
                ...
                if(nodeItem){
                    ...
                    setNodes(map => new Map(map.set((selectedItemsRef.current[0] as NodeItem).id, nodeItem)));
                    ToastNotification("success", "Vars set");
                }
            }
        }
    ...
    return (
        <div className="flex h-full grid grid-cols-12">
            <div className="h-full col-span-9" style={{ cursor: `url(${iconPaths[cursor]}) 15 15, crosshair` }} >
                <div style={{height:"90%"}}>
                    <PredictionMapImport twin={twinState.current} nodes={nodes} edges={edges} onSelectBuilding={onSelectBuilding}/>
                </div>
                <div className="flex justify-start gap-2">
                    <div className="bg-white gap-4 p-2 my-1 rounded-md flex justify-start">
                        ...
                        <Button outline={cursor !== CursorState.PLACE_TURBINE} onClick={(_: any) => toast("add preset (not implemented yet)")}>
                            <span className="whitespace-nowrap text-xl font-semibold dark:text-white"><Icon path={mdiPlus} size={1.2} /></span>
                        </Button>
                    </div>
                ...
                <div className="flex flex-col h-full">
                    <div
                        ...
                        {!selectedBuilding ? (
                            ...
                            <>
                                <div
                                    className={`text-lg font-semibold mb-4 ${!selectedBuilding.visible ? 'blur-sm' : ''}`}>
                                    Selected Building Details:
                                </div>
                                <div className={`text-gray-700 text-md ${!selectedBuilding.visible ? 'blur-sm' : ''}`}>
