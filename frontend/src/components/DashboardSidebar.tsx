...
function DashboardSidebar() {
    ...
    const SidebarItem = ({ children, href, label }: PropsWithChildren<{ href: string, label: string }>) => {
        const wrapperActiveStyles = pathName === href ? "bg-indigo-600 text-white" : "";
        const iconActiveStyles = pathName === href ? "text-white-800" : "";
        return <Link href={href} replace>
            <div className={`my-2 hover:bg-indigo-700 hover:text-white ${wrapperActiveStyles} flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 group`}>
                <div className={`w-5 h-5 transition duration-75 group-hover:text-white ${iconActiveStyles}`}>{children}</div>
    ...
    return (
        <div className="h-100 w-64 shadow-md z-40 bg-white flex flex-col">
            <div className="px-3 py-4 grow">
                ...
                <SidebarItem href="/dashboard/outage-monitoring" label="Outage Monitoring">
                </SidebarItem>
                <SidebarItem href="/dashboard/outage-diagnosis" label="Outage Diagnosis">
                </SidebarItem>
                <SidebarItem href="/dashboard/effect-analysis" label="Effect Analysis">
                </SidebarItem>
                <SidebarItem href="/dashboard/load-balancing" label="Load Balancing">
                </SidebarItem>
                <SidebarItem href="/dashboard/cost-reduction" label="Cost Reduction">
                </SidebarItem>
            ...
            <div className="m-2 flex flex-row">
                <Tooltip content="Logout">
                    <Button
                        ...
                        <CiLogout size={20} className="mx-4 group-hover:fill-white group-hover:border-white" />
                    </Button>
                </Tooltip>
                <Button
                    fullSized
